...
module tb_idu;
    ...
    task check_op;
        ...
        input [4:0] exp_rs2;
        input [31:0] exp_imm;
        input check_imm; // 1 to check imm, 0 to skip
    begin
        // 1. Apply Instruction
    ...
    initial begin
        ...
        check_op("LUI", 32'h123450B7, `CU_LUI, 5'd1, 5'd0, 5'd0, 32'h12345000, 1);

        // --- Type 1: AUIPC (CU Code 1) ---
        // AUIPC x2, 0x10000
        check_op("AUIPC", 32'h10000117, `CU_AUIPC, 5'd2, 5'd0, 5'd0, 32'h10000000, 1);

        // --- Type 2: JAL (CU Code 2) ---
        // JAL x1, offset -> J-Type
        // 000000000001 00000 000 00001 1101111 (JAL x1, 1) -> Imm decoding is complex
        check_op("JAL", 32'h001000EF, `CU_JAL, 5'd1, 5'd0, 5'd0, 32'h0, 0); // Imm is 0 in your code for JAL

        // --- Type 3: JALR (CU Code 3) ---
        // JALR x1, x2, 0
        check_op("JALR", 32'h000100E7, `CU_JALR, 5'd1, 5'd2, 5'd0, 32'h0, 1);

        // --- Type 4: Branch (BEQ - CU Code 4) ---
        // BEQ x1, x2, offset
        check_op("BEQ", 32'h00208063, `CU_BEQ, 5'd0, 5'd1, 5'd2, 32'h0, 0); // Checking op/regs only

        // --- Type 5: Store (SW - CU Code 12) ---
        // SW x2, 0(x1) -> funct3=010
        check_op("SW", 32'h0020A023, `CU_SW, 5'd0, 5'd1, 5'd2, 32'h0, 1);

        // --- Type 6: Load (LW - CU Code 15) ---
        // LW x1, 0(x2) -> funct3=010
        check_op("LW", 32'h00012083, `CU_LW, 5'd1, 5'd2, 5'd0, 32'h0, 1);

        // --- Type 7: Calc Imm (ADDI - CU Code 18) ---
        // ADDI x1, x2, 1 -> funct3=000
        check_op("ADDI", 32'h00110093, `CU_ADDI, 5'd1, 5'd2, 5'd0, 32'h1, 1);

        // --- Type 8: Register (ADD - CU Code 27) ---
        // ADD x1, x2, x3 -> funct3=000, funct7=0000000
        check_op("ADD", 32'h003100B3, `CU_ADD, 5'd1, 5'd2, 5'd3, 32'd0, 0);

        // --- Type 8: Register (SUB - CU Code 28) ---
        // SUB x1, x2, x3 -> funct3=000, funct7=0100000
        check_op("SUB", 32'h403100B3, `CU_SUB, 5'd1, 5'd2, 5'd3, 32'd0, 0);

        // --- Type 9: FENCE (CU Code 37) ---
        check_op("FENCE", 32'h0000000F, `CU_FENCE, 5'd0, 5'd0, 5'd0, 32'd0, 0);

        // --- Type 10: ECALL (CU Code 39) ---
        // Note: This might fail or behave oddly due to the 'instruction = 0' bug in DUT
        check_op("ECALL", 32'h00000073, `CU_ECALL, 5'd0, 5'd0, 5'd0, 32'd0, 0);
